...
.github/workflows/fix-pr-finding.yml -> tools/pr_loop.py, das ueberarbeite()
hier bei Bedarf mehrfach mit dem jeweils aktuellsten Befund aufruft, bis
Herr Brandt zufrieden ist oder die Versuchsobergrenze erreicht ist).
...
def ueberarbeite(finding: str) -> None:
...
Das ist bereits ein Korrekturversuch - der Text wird danach automatisch erneut streng geprueft, es gibt aber nur eine begrenzte Anzahl Versuche. Sei deshalb besonders sorgfaeltig: behebe nicht nur die genannte Stelle oberflaechlich, sondern pruefe den GESAMTEN Artikeltext und beide Social-Texte einmal komplett auf Konsistenz zueinander (Zahlen, Prozentangaben, Aussagen), bevor du antwortest - vermeide, dass eine Korrektur an einer Stelle eine neue Ungenauigkeit an anderer Stelle hinterlaesst.

Korrigiere GENAU das, was der Befund beschreibt, und stelle diese Konsistenz sicher - alles andere (Stil, Laenge, uebrige Inhalte) bleibt unveraendert. Halte dich an dieselben Regeln wie beim ersten Entwurf: Ich-Perspektive, Sie-Anrede (immer gross: Sie/Ihnen/Ihre), keine Behauptung jahrzehntelanger eigener Erfahrung, Artikeltext nur als h2/p/ul/li-HTML-Fragment (keine komplette Seite, kein html/head/body).
...
if __name__ == "__main__":
    _finding = os.environ.get("FINDING", "").strip()
    if not _finding:
        print("Fehler: FINDING Umgebungsvariable nicht gesetzt.")
        sys.exit(1)
    ueberarbeite(_finding)
