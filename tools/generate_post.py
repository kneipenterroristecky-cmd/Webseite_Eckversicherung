...
# ── Bild dynamisch suchen und KI-Auswahl ─────────────────────────────────────
_FALLBACK_IMG = "https://images.unsplash.com/photo-1554224155-6726b3ff858f"

def _find_best_image(topic_title, topic_label, topic_query, client, fallback_url, unsplash_key):
    """Sucht auf Unsplash und lässt Claude Vision das thematisch passendste Bild wählen."""
    if not unsplash_key:
        print("   ℹ️  Kein UNSPLASH_ACCESS_KEY – nutze topics.json Bild")
        return fallback_url
...
<style>
  * {{ margin: 0; padding: 0; box-sizing: border-box; }}
  ...
  h1 {{ word-break: break-word; font-size: 100px; font-weight: 900; color: #fff; line-height: 1.05; margin-bottom: 36px; letter-spacing: -.02em; text-shadow: 0 2px 20px rgba(0,0,0,.6); }}
  h1 span {{ color: #60a5fa; text-shadow: 0 2px 24px rgba(0,0,0,.95), 0 0 40px rgba(0,0,0,.7); }}
...
beide_html = social_html.replace("<strong>Daniel Eck</strong>", "<strong>Daniel &amp; Heiko Eck</strong>")
with open("social/latest-ig-beide.html", "w", encoding="utf-8") as f:
    f.write(beide_html)
print("✅ Social Image HTML erstellt: social/latest-ig-beide.html")
